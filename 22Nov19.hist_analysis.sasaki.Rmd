...
##操業データの入力
```{r}
User=1  # 1: sasaki 2:gaku
if(User==1){setwd("C:/Users/sasaki1623/Dropbox/Sasaki/Data")}else if
(User==2){ setwd("/Users/gakugaku/Dropbox/#_0_gakuGroup/gakugroup_Sasaki_Share/Data")}
#base_data
if(User==1){data1<-read.csv("14Jun19_Operation_Data.2012_2018.sasaki.csv",
                            stringsAsFactors=FALSE,header=T,na.strings="NULL")}else if 
(User==2){data1<-read.csv("14Jun19_Operation_Data.2012_2018.sasaki.csv",
                          stringsAsFactors=FALSE,header=T,na.strings="NULL",
                          fileEncoding="UTF-8-BOM")}   
```

##market_dataの入力
```{r}
if(User==1){M_data1<-read.csv("kesennuma_market_20090401_20180331.csv",
                            stringsAsFactors=FALSE,header=T,skip=1,na.strings="NULL")}else if 
(User==2){M_data1<- read_excel("NEW_kesennuma_market_20090401_20180331.xlsx")} 

#魚種の英名
if(User==1){English_Names <- 
  read.csv("10May18FishName_gaku_checked.csv",header=T)}else if
  (User==2){English_Names <-     
    read_excel("3May19FishName.xlsx")}
```

##Fukuyo_8におけるmarket_data入力
```{r}
if(User==1){Fukuyo_data<-read.csv("6May19.Fukuyo_8.market_data.csv",
                            stringsAsFactors=FALSE,header=T,na.strings="NULL")}else if 
(User==2){Fukuyo_data<-read.csv("6May19.Fukuyo_8.market_data.csv",
                          stringsAsFactors=FALSE,header=T,na.strings="NULL",
                          fileEncoding="UTF-8-BOM")} 
...
##漁船名セットの入力
```{r}
if(User==1){Vessel_Name_Match<-read.csv("6May.19.Vessel_Name_Match.csv",
                            stringsAsFactors=FALSE,header=T,na.strings="NULL")}else if 
(User==2){Vessel_Name_Match<-read.csv("6May.19.Vessel_Name_Match.csv",
                          stringsAsFactors=FALSE,header=T,na.strings="NULL",
                          fileEncoding="UTF-8-BOM")} 
```
...
M_data3$Fish_Names2 <- factor(M_data3$Fish_Names,
                                     levels = c("Swordfish","Blue_Shark"))

M_data7<-dplyr::filter(M_data3, Year_Landing ==2015) 
w <- ggplot(M_data7,aes(x = Month_Landing, y = Weight_kg_sum/1000))
w <- w + geom_bar(stat = "identity",aes(fill=Fish_Names2))
w <- w + labs(x = "月", y = "漁獲量(t)", title = "月別の魚種構成")
w <- w + scale_x_continuous(breaks = seq(1,12, by = 1)) 

v <- ggplot(M_data7,aes(x = Month_Landing, y = Value_JPY_sum/1000000))
v <- v + geom_bar(stat = "identity",aes(fill=Fish_Names2))
v <- v + labs(x = "月", y = "漁獲高(百万円)", title = "月別の魚種構成")
v <- v + scale_x_continuous(breaks = seq(1,12, by = 1))
...
```{r}
w <- ggplot(M_data9,aes(x = M_data9$cluster3, y = Weight_kg_sum/1000))
w <- w + geom_bar(stat = "identity",aes(fill=Fish_Names2))
w <- w + labs(x = "クラスター", y = "漁獲量(t)", 
              title = "クラスター別の魚種構成")
w <- w + scale_x_continuous(breaks = seq(1,3, by = 1)) 

v <- ggplot(M_data9,aes(x = M_data9$cluster3, y = Value_JPY_sum/1000000))
v <- v + geom_bar(stat = "identity",aes(fill=Fish_Names2))
v <- v + labs(x = "クラスター", y = "漁獲高(百万円)", 
              title = "クラスター別の魚種構成")
v <- v + scale_x_continuous(breaks = seq(1,3, by = 1))
grid.arrange(w,v,ncol = 2)
...
```{r}
M_data9$cluster4 <- as.numeric(M_data9$cluster4)
w <- ggplot(M_data9,aes(x = M_data9$cluster4, y = Weight_kg_sum/1000))
w <- w + geom_bar(stat = "identity",aes(fill=Fish_Names2),
                  position = "fill")
w <- w + labs(x = "クラスター", y = "漁獲量(t)", 
              title = "クラスター別の魚種構成")+
...
v <- ggplot(M_data9,aes(x = M_data9$cluster4, y = Value_JPY_sum/1000000))
v <- v + geom_bar(stat = "identity",aes(fill=Fish_Names2),
                  position = "fill")
v <- v + labs(x = "クラスター", y = "漁獲高(百万円)", 
              title = "クラスター別の魚種構成")+
...
g <- g + geom_histogram(position = "identity", alpha = 0.4)+scale_y_continuous(trans = "log1p")
g <- g + scale_fill_npg()+ labs(x = "漁獲量(t)")
plot(g)
```
...
g2 <- g2 + geom_density(aes(color =cluster2, alpha = 0.2),
                      show.legend = F)
g2 <- g2 + labs(x = "漁獲高(万円)")
g2 <- g2 + scale_fill_npg()+ scale_color_npg()
...
g2 <- ggplot(M_data9, aes(x = Value_JPY_sum/10000,y = ..density..,
                         fill = cluster2))
g2 <- g2 + geom_histogram(position = "dodge")
g2 <- g2 + scale_fill_npg()+ scale_color_npg()
grid.arrange(g,g2,ncol = 1)
```
